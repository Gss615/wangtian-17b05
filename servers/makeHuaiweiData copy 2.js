    ...
    const result = _.chain(datas).compact()
                                ...
                                .map(item=>{
                                    ...
                                    if(item.imgs === '没有图片'){
                                        let  temp = name.split(' ')
                                        item.name2 = temp[1]
                                        item.imgs = allFiles[temp[1]] || '没有图片'
                                    }
                                    if(item.imgs === '没有图片'){
                                        let  temp = name.split('-')
                                        item.list1 = temp
                                         item.name3 =  _.chain(allFiles).keys().filter(item=>temp[0] && item.indexOf(temp[0])>-1).head().value()
                                         item.imgs = allFiles[item.name3] || '没有图片'
                                         item.list =  _.chain(allFiles).keys().filter(item=>temp[0] && (item.indexOf(temp[0])>-1)).value()
